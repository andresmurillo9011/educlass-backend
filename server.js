...
//  SALUD
// ======================================================
app.get("/", (_req, res) => res.json({ ok:true, msg:"EduClass v5 ✅" }));
...
app.get("/estudiantes-grado/:grado", (req, res) => {
  try {
    ...
    const lista = db.estudiantesReg
      .filter(e => e.grado === grado ||
                   e.grado === grado.replace("°","") ||
                   (e.grado+"°") === grado)
  ...
  try {
    ...
    const tareasAsignadas = db.tareas.filter(t => {
      ...
      return {
        ...
        // Solo mostramos la calificación, NO el detalle de respuestas
        calificacion:entrega?.calificacion||null,
        comentario:entrega?.comentarioDocente||entrega?.comentario||""
      };
    });
...
app.post("/crear-tarea", (req, res) => {
  try {
    const {
      ...
      asignarGrado,       // "manual" | "6°" | ...
      estudiantesRegIds   // IDs seleccionados individualmente
    } = req.body;
    ...
    // Estudiantes registrados por grado completo
    ...
    // Estudiantes seleccionados individualmente
    ...
    const tarea = {
      ...
      codigo,
      estudiantesAsignados: [],  // legacy (lista manual)
  ...
  try {
    ...
    const tareas = db.tareas
      ...
      .map(t => {
        ...
        const pendientes = total - entregas.length;
        return { ...t, totalEntregas:entregas.length, totalEstudiantes:total, pendientes:Math.max(0,pendientes) };
      });
    res.json({ tareas });
  } catch(e) { res.status(500).json({ mensaje:e.message }); }
});

// ── Resumen completo de entregas (panel docente) ───────
app.get("/entregas-tarea/:tareaId", (req, res) => {
  try {
    ...
    const tempAsignados = (tarea.estudiantesAsignados||[]).map(e => ({
      id:e.id, nombre:e.nombre, grado:tarea.grado
    }));

    const todosAsignados = [...regAsignados, ...tempAsignados];

    // Listado completo con estado — SIN exponer detalle de respuestas al docente tampoco
...
app.post("/calificar-entrega", (req, res) => {
  try {
    const { entregaId, calificacion, comentarioDocente } = req.body;
    const db  = leerDB();
    ...
    db.entregas[idx].comentarioDocente = comentarioDocente||"";
...
app.post("/generar-actividad", async (req, res) => {
  try {
    ...
    const prompts = {
      ...
      completar:       `Genera ${n} oraciones completar espacio sobre "${tema}" ${area} grado ${grado}°. JSON: {"preguntas":[{"enunciado":"___","respuesta":""}]}. Solo JSON.`,
      ...
      taller:          `Genera ${n} preguntas abiertas taller sobre "${tema}" ${area} grado ${grado}°. JSON: {"preguntas":[{"pregunta":"","tipo":"abierta"}]}. Solo JSON.`,
      evaluacion:      `Genera evaluación mixta sobre "${tema}" ${area} grado ${grado}°: 3 selección múltiple + 2 V/F + 1 abierta. JSON: {"preguntas":[{"tipo":"seleccion","pregunta":"","opciones":["A.","B.","C.","D."],"correcta":"A"}]}. Solo JSON.`,
    };
...
DATOS: Institución:${institucion||"I.E."} | Docente:${docente||"Docente"} (${cargo||"Docente"}) | Ciudad:${ciudad||"Colombia"} | Área:${area} | Grado:${grado}° | Periodo:${periodo||"1"} | Fecha:${fecha||new Date().toLocaleDateString("es-CO")} | Tema:${tema} | Duración:${durTexto}
...
===APERTURA===
TIPO: ${tipoApertura}
Redacta la apertura completa contextualizada en Colombia, nivel ${nivelEducativo||nivel}, grado ${grado}°. Mínimo 8 líneas.
...
Contenido temático MUY COMPLETO sobre "${tema}" para ${nivelEducativo||nivel} grado ${grado}°.
Mínimo 5 subtemas con ## cada uno. Ejemplos reales de Colombia. Preguntas reflexivas.
...
===TAREA===
${dejaTarea?`ESTRATEGIA: ${estratTarea}\nRedacta la tarea.`:"Sin tarea para esta sesión."}

===EXTRAS===
OBJETIVO: (objetivo completo con verbo infinitivo para "${tema}" nivel ${nivelEducativo||nivel} grado ${grado}°)
COMPETENCIA: (competencia específica área ${area} grado ${grado}°)
...
CRITERIO: (escala Bajo/Básico/Alto/Superior)
...
Redacta en español impecable. Todo contextualizado Colombia. NUNCA genérico.`;
    ...
    const resp = await groq.chat.completions.create({
      ...
      messages:[
        { role:"system", content:"Eres el mejor pedagogo de Colombia. Guías altísima calidad. Respeta ===SECCION===." },
        { role:"user",   content:prompt }
      ]
    });
    res.json({ contenido:resp.choices[0].message.content, ok:true });
  } catch(e) { console.error("❌ Groq:",e.message); res.status(500).json({ mensaje:e.message }); }
});
...
app.post("/exportar-word", async (req, res) => {
  try {
    ...
    const secs=[{t:"INICIO (APERTURA Y MOTIVACIÓN)",c:getB("APERTURA").substring(0,800)},{t:"EXPLORACIÓN (SABERES PREVIOS)",c:getB("SABERES_PREVIOS").split("\n").filter(l=>l.trim()).map((l,i)=>(i+1)+". "+l.replace(/^\d+\.\s*/,"")).join("\n").substring(0,800)},{t:"ESTRUCTURACIÓN (DESARROLLO Y TALLER)",c:(getB("DESARROLLO")+" "+getB("TALLER")).substring(0,1200)},{t:"TRANSFERENCIA (RETROALIMENTACIÓN)",c:getB("RETROALIMENTACION").substring(0,1000)},{t:"REFUERZO (CIERRE Y TAREA)",c:(getB("CIERRE")+"\n"+getB("TAREA")).substring(0,1000)}];
...
// ======================================================
app.listen(5000, () => {
  console.log("✅ EduClass Premium v5 — Puerto 5000");
