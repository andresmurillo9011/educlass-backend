...
const leerDB = () => {
  ...
  // Garantizar que todos los campos existen (migración segura)
...
//  SALUD
// ======================================================
app.get("/", (_req, res) => res.json({ ok:true, msg:"EduClass Premium IA v4 ✅" }));
...
app.post("/registro", async (req, res) => {
  try {
    ...
    if (!nombre || !email || !password)
      return res.status(400).json({ mensaje:"Completa nombre, correo y contraseña" });
    const db = leerDB();
    if (db.usuarios.find(u => u.email === email))
      return res.status(400).json({ mensaje:"Este correo ya está registrado" });
    const hash = await bcrypt.hash(password, 10);
    db.usuarios.push({
      id:uuidv4(), nombre, email, password:hash,
      institucion:institucion||"", cargo:cargo||"Docente",
      ciudad:ciudad||"", municipio:municipio||"",
      logoPath:"", banderaPath:"",
...
// ======================================================
//  ESTUDIANTES — Registro y login propio
// ======================================================
...
app.post("/registro-estudiante", async (req, res) => {
  try {
    ...
    if (db.estudiantesReg.find(e => e.usuario === usuario))
      return res.status(400).json({ mensaje:"Ese usuario ya existe, elige otro" });
    const hash = await bcrypt.hash(password, 10);
    const nuevo = {
      id:uuidv4(), nombre, usuario, password:hash,
      grado:grado||"", institucion:institucion||"",
      creadoEn:new Date().toISOString()
    };
...
app.post("/login-estudiante-reg", async (req, res) => {
  try {
    ...
    if (!est) return res.status(401).json({ mensaje:"Usuario no encontrado. Regístrate primero o verifica el usuario." });
...
// ── Listar TODOS los estudiantes registrados (para docente al crear tarea) ──
...
// Listar estudiantes filtrados por grado
app.get("/estudiantes-grado/:grado", (req, res) => {
  try {
    const db    = leerDB();
    const grado = req.params.grado;
    const lista = db.estudiantesReg
      .filter(e => e.grado === grado || e.grado === grado.replace("°",""))
...
// Grados disponibles de estudiantes registrados
...
app.post("/crear-tarea", (req, res) => {
  try {
    const {
      docenteId, titulo, descripcion, tipo, actividad,
      area, grado, fechaEntrega,
      estudiantesLista,   // lista manual (nombres)
      asignarGrado,       // "manual" | "6°" | etc.
      estudiantesRegIds   // IDs de estudiantesReg seleccionados individualmente
    } = req.body;
    ...
    // Cuentas temporales para lista manual
    ...
    // Estudiantes registrados por grado o selección individual
    const estudiantesRegAsignados = [];
    if (asignarGrado && asignarGrado !== "manual") {
      const gradoNum = asignarGrado.replace("°","");
      db.estudiantesReg
        .filter(e => e.grado === gradoNum || e.grado === asignarGrado)
    ...
    const tareas = db.tareas
      ...
      .map(t => {
        ...
        return { ...t, totalEntregas:entregas.length, totalEstudiantes:total,
                 pendientes:total - entregas.length };
      });
    res.json({ tareas });
  } catch(e) { res.status(500).json({ mensaje:"Error: "+e.message }); }
});

// ── Ver entregas con LISTADO COMPLETO: quién entregó y quién no ──
app.get("/entregas-tarea/:tareaId", (req, res) => {
  try {
    ...
    // Estudiantes registrados asignados
    const regAsignados = (tarea.estudiantesReg||[]).map(id => {
      const est = db.estudiantesReg.find(e => e.id === id);
      return est ? { id:est.id, nombre:est.nombre, tipo:"registrado" } : null;
    }).filter(Boolean);

    // Estudiantes temporales (lista manual)
    const tempAsignados = (tarea.estudiantesAsignados||[]).map(e => ({
      id:e.id, nombre:e.nombre, tipo:"temporal"
    }));

    const todosAsignados = [...regAsignados, ...tempAsignados];

    // Construir listado completo con estado
    const listadoCompleto = todosAsignados.map(est => {
      const entrega = entregas.find(en =>
        en.estudianteRegId === est.id || en.estudianteId === est.id
      );
      return {
        estudianteId:    est.id,
        ...
        respuestasActividad: entrega?.respuestasActividad || null,
        resultadoDetalle:    entrega?.resultadoDetalle || null,
        ...
        autoCalificada:  entrega?.autoCalificada || false,
        estado:          entrega
          ? (entrega.calificacion ? "calificado" : "entregado")
          : "pendiente"
      };
    });

    const entregaron   = listadoCompleto.filter(e => e.entregada);
    ...
    res.json({
      ...
      totalEntregas:  entregaron.length,
...
app.post("/calificar-entrega", (req, res) => {
  try {
    ...
    db.entregas[idx].autoCalificada = false;
    guardarDB(db);
...
// Login estudiante temporal (lista manual)
app.post("/login-estudiante", async (req, res) => {
  try {
    ...
    res.json({ ok:true, estudiante:{ id:est.id, nombre:est.nombre, usuario:est.usuario },
               tarea, yaEntrego:!!entrega, entrega:entrega||null });
  } catch(e) { res.status(500).json({ mensaje:"Error: "+e.message }); }
});
...
app.post("/entregar-tarea", uploadEntrega.single("archivo"), (req, res) => {
  try {
    ...
    if (tarea && respAct && tarea.actividad?.preguntas &&
        ["quiz","completar","verdadero_falso"].includes(tarea.tipo)) {
    ...
    const entrega = {
      ...
      comentario:      autoCalificada
        ? `Auto: ${resultadoDetalle?.correctas}/${resultadoDetalle?.total} correctas (${resultadoDetalle?.porcentaje}%)`
        : (idx>=0 ? db.entregas[idx].comentario : ""),
...
app.get("/descargar-entrega/:tareaId/:estudianteId", (req, res) => {
  try {
    ...
    if (!ent?.archivoPath) return res.status(404).json({ mensaje:"Archivo no encontrado" });
    const abs = path.join(__dirname, ent.archivoPath);
    if (!fs.existsSync(abs)) return res.status(404).json({ mensaje:"Archivo eliminado del servidor" });
...
app.post("/generar-actividad", async (req, res) => {
  try {
    ...
    const n = cantidad||5;
    const prompts = {
      quiz:            `Genera un quiz de ${n} preguntas de selección múltiple sobre "${tema}" para ${area} grado ${grado}° en Colombia. Formato JSON: {"preguntas":[{"pregunta":"texto","opciones":["A. texto","B. texto","C. texto","D. texto"],"correcta":"A"}]}. Solo JSON.`,
      completar:       `Genera ${n} oraciones para completar el espacio sobre "${tema}" para ${area} grado ${grado}°. Formato JSON: {"preguntas":[{"enunciado":"La ___ es...","respuesta":"palabra"}]}. Solo JSON.`,
      verdadero_falso: `Genera ${n} afirmaciones V/F sobre "${tema}" para ${area} grado ${grado}°. Formato JSON: {"preguntas":[{"afirmacion":"texto","respuesta":"Verdadero"}]}. Solo JSON.`,
      relacionar:      `Genera ${n} pares para relacionar sobre "${tema}" para ${area} grado ${grado}°. Formato JSON: {"pares":[{"columnaA":"concepto","columnaB":"definición"}]}. Solo JSON.`,
      taller:          `Genera ${n} preguntas abiertas de taller sobre "${tema}" para ${area} grado ${grado}°. Formato JSON: {"preguntas":[{"pregunta":"texto","tipo":"abierta"}]}. Solo JSON.`,
      evaluacion:      `Genera evaluación mixta sobre "${tema}" para ${area} grado ${grado}°: 3 selección múltiple, 2 completar, 2 V/F, 1 abierta. Formato JSON: {"preguntas":[{"numero":1,"tipo":"seleccion","pregunta":"texto","opciones":["A. x","B. y","C. z","D. w"],"correcta":"A"}]}. Solo JSON.`,
    };
    ...
    catch { res.json({ ok:true, actividad:{ preguntas:[] }, tipo, raw:txt }); }
  } catch(e) { res.status(500).json({ mensaje:"Error: "+e.message }); }
});
...
===APERTURA===
TIPO: ${tipoApertura}
Redacta el contenido completo de la apertura contextualizado en Colombia. Mínimo 8 líneas.

===SABERES_PREVIOS===
1. (pregunta diagnóstica 1 sobre "${tema}")
2. (pregunta diagnóstica 2 sobre "${tema}")
3. (pregunta diagnóstica 3 sobre "${tema}")
4. (pregunta diagnóstica 4 sobre "${tema}")

===DESARROLLO===
ESTRATEGIA: ${estratDesarrollo}
Redacta el contenido temático MUY COMPLETO sobre "${tema}" para ${nivelEducativo||nivel}, grado ${grado}°.
Mínimo 5 subtemas con ## cada uno. Ejemplos reales de Colombia. Preguntas reflexivas entre ** **.

===RETROALIMENTACION===
ESTRATEGIA: ${retroalimentacion}
Redacta la actividad completa de retroalimentación sobre "${tema}".
...
5. INVESTIGACIÓN: (1 consulta breve con fuente)

===CIERRE===
ESTRATEGIA: ${tipoCierre}
Redacta el cierre con síntesis, metacognición y conexión con la vida cotidiana.

===TAREA===
${dejaTarea?`ESTRATEGIA: ${estratTarea}\nRedacta la tarea creativa y factible.`:"Sin tarea para esta sesión."}

===EXTRAS===
OBJETIVO: (objetivo completo con verbo en infinitivo para "${tema}")
COMPETENCIA: (competencia específica del área ${area} grado ${grado}°)
DBA: (DBA completo MEN Colombia para ${area} grado ${grado}°)
ESTANDAR: (Estándar básico de competencia MEN completo)
INDICADOR1: (indicadores SABER — niveles 🟢Básico 🟡Intermedio 🔵Avanzado)
INDICADOR2: (indicadores HACER — mismos niveles)
INDICADOR3: (indicadores SER — mismos niveles)
EVIDENCIA: (evidencia de aprendizaje observable)
CRITERIO: (escala: Bajo / Básico / Alto / Superior con descripción)
...
Redacta en español impecable. Todo contextualizado Colombia. NUNCA genérico.`;

    const resp = await groq.chat.completions.create({
      model:"llama-3.3-70b-versatile", max_tokens:4096, temperature:0.7,
      messages:[
        { role:"system", content:"Eres el mejor pedagogo de Colombia. Guías altísima calidad. NUNCA genérico. Respeta ===SECCION===." },
...
app.post("/exportar-word", async (req, res) => {
  try {
    ...
    const durMin=parseInt(duracion)||55;const numHoras=Math.round(durMin/55);const durTexto=numHoras===1?"1 hora (55 min)":`${numHoras} horas (${durMin} min)`;
    ...
    const cel=(children,opts={})=>new TableCell({width:{size:opts.w||1000,type:WidthType.DXA},margins:{top:80,bottom:80,left:100,right:100},shading:{fill:opts.fill||BLANCO,type:ShadingType.CLEAR},verticalAlign:opts.va||VerticalAlign.CENTER,borders:opts.borders||bAll,children});
    const makeLogoCell=(imgPath,w)=>{const abs=imgPath?path.join(__dirname,imgPath):null;if(abs&&fs.existsSync(abs)){try{const buf=fs.readFileSync(abs);return cel([par([new ImageRun({data:buf,transformation:{width:80,height:80},type:"png"})],{align:AlignmentType.CENTER})],{w,fill:BLANCO,borders:bAllA});}catch(_){}}return cel([par([txt("",{size:20})])],{w,fill:AZUL_CL,borders:bAllA});};
    const children=[];const LC=1300,RC=1300,CC=TW-LC-RC;
    children.push(new Table({width:{size:TW,type:WidthType.DXA},columnWidths:[LC,CC,RC],borders:{top:bA,bottom:bA,left:bA,right:bA,insideH:bA,insideV:bA},rows:[new TableRow({children:[makeLogoCell(logoPath,LC),cel([par([txt((institucion||"INSTITUCIÓN EDUCATIVA").toUpperCase(),{bold:true,size:24,color:AZUL})],{align:AlignmentType.CENTER,spacing:{before:60,after:20}}),par([txt("Aprobado según Decreto No. 0001295 del 04 Noviembre de 2009",{size:16})],{align:AlignmentType.CENTER,spacing:{before:0,after:0}}),par([txt("De la Secretaría de Educación del Caquetá",{size:16})],{align:AlignmentType.CENTER,spacing:{before:0,after:0}}),par([txt(`${ciudad||"Valparaíso"} - Caquetá`,{size:18,bold:true})],{align:AlignmentType.CENTER,spacing:{before:20,after:40}})],{w:CC,borders:bAllA}),makeLogoCell(banderaPath,RC)]})]})  );
    ...
    children.push(new Table({width:{size:TW,type:WidthType.DXA},columnWidths:[c1,c2,c3,c4,c5,c6,c7,c8,c9,c10v],rows:[new TableRow({children:[cel([par([txt("NIVEL",{bold:true,size:18})])],{w:c1,fill:GRIS,borders:bAll}),cel([par([txt(nivelLabel,{size:18})])],{w:c2,borders:bAll}),cel([par([txt("GRADO",{bold:true,size:18})])],{w:c3,fill:GRIS,borders:bAll}),cel([par([txt(grado||"",{size:18})])],{w:c4,borders:bAll}),cel([par([txt("PERÍODO",{bold:true,size:18})])],{w:c5,fill:GRIS,borders:bAll}),cel([par([txt(periodo||"I",{size:18})])],{w:c6,borders:bAll}),cel([par([txt("ÁREA",{bold:true,size:18})])],{w:c7,fill:GRIS,borders:bAll}),cel([par([txt(area||"",{size:18})])],{w:c8,borders:bAll}),cel([par([txt("SEM",{bold:true,size:18})])],{w:c9,fill:GRIS,borders:bAll}),cel([par([txt("1",{size:18})])],{w:c10v,borders:bAll})]})]})  );
    const LBL=Math.round(TW*.13),VAL=TW-Math.round(TW*.13);const fila2=(lb,vl)=>new Table({width:{size:TW,type:WidthType.DXA},columnWidths:[LBL,VAL],rows:[new TableRow({children:[cel([par([txt(lb,{bold:true})])],{w:LBL,fill:GRIS,borders:bAll}),cel([par([txt(vl)])],{w:VAL,borders:bAll})]})]});
    const ex=getBloque("EXTRAS");
    const obj=ex.match(/OBJETIVO[:\s\n]+(Desarrollar|Identificar|Comprender|Analizar|Aplicar|Producir|Utilizar|Reconocer|Construir|Explorar|Crear|Demostrar|Evaluar|Fomentar)[\s\S]*?(?=\nCOMPETENCIA:|\nESTANDAR:|\nDBA:|\nINDICADOR|$)/i)?.[0]?.replace(/^OBJETIVO[:\s]*/i,"")?.trim()||"Desarrollar competencias en los estudiantes.";
    const rec=ex.match(/RECURSOS[:\s]+([\s\S]*?)(?=WEBGRAFIA:|$)/i)?.[1]?.trim()||"Talento humano, cuaderno, lápiz, colores.";
    ...
    children.push(fila2("TEMA",tema));children.push(fila2("OBJETIVO",obj.substring(0,400)));children.push(fila2("RECURSOS",recursos.substring(0,400)));children.push(fila2("WEBGRAFÍA",web.substring(0,400)));
    ...
    const LR=Math.round(TW*.28),VR=TW-Math.round(TW*.28);
    children.push(new Table({width:{size:TW,type:WidthType.DXA},columnWidths:[LR,VR],rows:[new TableRow({children:[cel([par([txt("ESTÁNDAR BÁSICO DE COMPETENCIA (MEN)",{bold:true})])],{w:LR,fill:GRIS,borders:bAll}),cel([par([txt(std.substring(0,400))])],{w:VR,borders:bAll})]}),new TableRow({children:[cel([par([txt("DBA — DERECHO BÁSICO DE APRENDIZAJE",{bold:true})])],{w:LR,fill:GRIS,borders:bAll}),cel([par([txt(dba.substring(0,400))])],{w:VR,borders:bAll})]})]}));
    const S1=Math.round(TW*.18),S2=Math.round((TW-Math.round(TW*.18))/3),S3=S2,S4=TW-S1-S2-S3;
    children.push(new Table({width:{size:TW,type:WidthType.DXA},columnWidths:[S1,S2,S3,S4],rows:[new TableRow({children:[cel([par([txt("INDICADORES DE COMPETENCIA",{bold:true})])],{w:S1,fill:GRIS,borders:bAll}),cel([par([txt("SABER",{bold:true})],{align:AlignmentType.CENTER})],{w:S2,fill:GRIS,borders:bAll}),cel([par([txt("HACER",{bold:true})],{align:AlignmentType.CENTER})],{w:S3,fill:GRIS,borders:bAll}),cel([par([txt("SER",{bold:true})],{align:AlignmentType.CENTER})],{w:S4,fill:GRIS,borders:bAll})]}),new TableRow({children:[cel([par([txt("Por dimensión")])],{w:S1,borders:bAll}),cel([par([txt(i1.substring(0,600))])],{w:S2,borders:bAll}),cel([par([txt(i2.substring(0,600))])],{w:S3,borders:bAll}),cel([par([txt(i3.substring(0,400))])],{w:S4,borders:bAll})]})]}));
    ...
    const secciones=[{t:"INICIO DE LA CLASE (APERTURA Y MOTIVACIÓN)",c:getBloque("APERTURA").substring(0,800)},{t:"EXPLORACIÓN (SABERES PREVIOS)",c:getBloque("SABERES_PREVIOS").split("\n").filter(l=>l.trim()).map((l,i)=>(i+1)+". "+l.replace(/^\d+\.\s*/,"")).join("\n").substring(0,800)},{t:"ESTRUCTURACIÓN (PRÁCTICA Y DESARROLLO)",c:(getBloque("DESARROLLO")+" "+getBloque("TALLER")).substring(0,1200)},{t:"TRANSFERENCIA (VALORACIÓN)",c:getBloque("RETROALIMENTACION").substring(0,1000)},{t:"REFUERZO (INTEGRACIÓN A CONTEXTOS COTIDIANOS)",c:(getBloque("CIERRE")+"\n"+getBloque("TAREA")).substring(0,1000)}];
    for(const s of secciones){children.push(new Table({width:{size:TW,type:WidthType.DXA},columnWidths:[TW],rows:[new TableRow({children:[cel([par([txt(s.t,{bold:true})])],{w:TW,fill:GRIS,borders:bAll})]}),new TableRow({children:[cel([par([txt(s.c||".")])],{w:TW,borders:bAll})]})]})  );}
    ...
    const EV=Math.round(TW*.6),ET=TW-Math.round(TW*.6);
...
app.post("/exportar-pdf", async (req, res) => {
  try {
    ...
    for(const linea of contenido.split("\n")){const m=linea.match(/^===(\w+)===/);if(m){secActual=m[1];bloques[secActual]=[];continue;}if(secActual)bloques[secActual].push(linea);}
    const getBloque=k=>(bloques[k]||[]).join("\n").replace(/\*\*/g,"").replace(/^[\s\n]+|[\s\n]+$/g,"");
    const durMin=parseInt(duracion)||55;const numHoras=Math.round(durMin/55);
    ...
    const border=()=>{doc.rect(4,4,doc.page.width-8,doc.page.height-8).lineWidth(2).strokeColor(AZUL).stroke();doc.rect(7,7,doc.page.width-14,doc.page.height-14).lineWidth(0.5).strokeColor(AZUL_CL).stroke();};
    border();
    ...
    if(logoAbs&&fs.existsSync(logoAbs)){try{doc.image(logoAbs,ML+3,HY+4,{width:78,height:78,fit:[78,78]});}catch(_){doc.rect(ML+3,HY+4,78,78).fill(AZUL_CL);}}else{doc.rect(ML+3,HY+4,78,78).fill(AZUL_CL);doc.font(FB).fontSize(7).fillColor(AZUL).text("ESCUDO",ML+3,HY+37,{width:78,align:"center"});}
    const banAbs=banderaPath?path.join(__dirname,banderaPath):null;
    if(banAbs&&fs.existsSync(banAbs)){try{doc.image(banAbs,ML+PW-81,HY+4,{width:78,height:78,fit:[78,78]});}catch(_){doc.rect(ML+PW-81,HY+4,78,78).fill(AZUL_CL);}}else{doc.rect(ML+PW-81,HY+4,78,78).fill(AZUL_CL);doc.font(FB).fontSize(7).fillColor(AZUL).text("BANDERA",ML+PW-81,HY+37,{width:78,align:"center"});}
    ...
    const ex=getBloque("EXTRAS");
    const obj=ex.match(/OBJETIVO[:\s\n]+(Desarrollar|Identificar|Comprender|Analizar|Aplicar|Producir|Utilizar|Reconocer|Construir|Explorar|Crear|Demostrar|Evaluar|Fomentar)[\s\S]*?(?=\nCOMPETENCIA:|\nESTANDAR:|\nDBA:|\nINDICADOR|$)/i)?.[0]?.replace(/^OBJETIVO[:\s]*/i,"")?.trim()||"Desarrollar competencias.";
    ...
    const i1=ex.match(/INDICADOR1[:\s]+([\s\S]*?)(?=INDICADOR2:|$)/i)?.[1]?.trim()||"Comprende.";
    const i2=ex.match(/INDICADOR2[:\s]+([\s\S]*?)(?=INDICADOR3:|$)/i)?.[1]?.trim()||"Aplica.";
    const i3=ex.match(/INDICADOR3[:\s]+([\s\S]*?)(?=EVIDENCIA:|$)/i)?.[1]?.trim()||"Actitud.";
    const dba=ex.match(/DBA[:\s\n]+([\s\S]*?)(?=ESTANDAR:|INDICADOR|EVIDENCIA:|CRITERIO:|RECURSOS:|WEBGRAFIA:|$)/i)?.[1]?.trim()||"Ver DBA MEN.";
    const std=ex.match(/ESTANDAR[:\s\n]+([\s\S]*?)(?=DBA:|INDICADOR|EVIDENCIA:|CRITERIO:|$)/i)?.[1]?.trim()||"Ver estándares MEN.";
    const evd=ex.match(/EVIDENCIA[:\s]+([\s\S]*?)(?=CRITERIO:|RECURSOS:|$)/i)?.[1]?.trim()||"Demuestra comprensión.";
    tabla([[gr("Nombre del Docente:",{fs:8}),nm(docente||"",{fs:8}),gr("Sede:",{fs:8}),nm(institucion||"",{fs:8})]],[PW*.15,PW*.35,PW*.1,PW*.4]);
    tabla([[az("INFORMACIÓN GENERAL")]],[PW]);
    tabla([[gr("NIVEL",{fs:8}),nm(nivelLabel,{fs:8}),gr("GRADO",{fs:8}),nm(grado||"",{fs:8}),gr("PERÍODO",{fs:8}),nm(periodo||"",{fs:8}),gr("ÁREA",{fs:8}),nm(area||"",{fs:8}),gr("SEM",{fs:8}),nm("1",{fs:8})]],[PW*.07,PW*.12,PW*.07,PW*.07,PW*.08,PW*.07,PW*.07,PW*.28,PW*.06,PW*.11]);
    ...
    tabla([[gr("ESTÁNDARES BÁSICOS",{fs:8}),nm(std.substring(0,250),{fs:8})],[gr("DBA",{fs:8}),nm(dba.substring(0,250),{fs:8})]],[PW*.22,PW*.78]);
...
// ======================================================
app.listen(5000, () => {
  console.log("✅ EduClass Premium IA v4 — Puerto 5000 activo");
