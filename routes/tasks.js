...
router.post("/entregar", authEstudiante, async (req, res) => {
  try {
    ...
    // ✅ FIX: Parsear activity si viene como string JSON
    let actividadObj = assignment.task.activity;
    if (typeof actividadObj === "string") {
      try { actividadObj = JSON.parse(actividadObj); } catch(_) { actividadObj = {}; }
    }

    if (["QUIZ","COMPLETAR","VERDADERO_FALSO"].includes(tipo) && actividadObj?.preguntas) {
      const preguntas = actividadObj.preguntas;
